...
public class ChatChannelManager extends Manager<ChatChannel> {
    ...
    public void setChannel(UserPlayer userPlayer, String key){
        ...
        if(channel.isPresent() && channel.get() != userPlayer.getChatChannel()){
            ...
            Messager.sendMessage(userPlayer.getPlayer().get(), Text.of(TextColors.GRAY, "Moved to chat channel: " + channel.get().getKey()), Optional.of(Messager.Prefix.SUCCESS));
        } else {
            Messager.sendMessage(userPlayer.getPlayer().get(), Text.of(TextColors.GRAY, "The channel doesn't exist or you are already in it: " + key), Optional.of(Messager.Prefix.ERROR));
        }
    }
